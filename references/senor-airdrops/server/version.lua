local updateUrl = 'https://raw.githubusercontent.com/darealsenor/senor-airdrop-fivem/refs/heads/main/version'
local discordUrl = 'https://raw.githubusercontent.com/darealsenor/senor-scripts-related/refs/heads/main/discord'
local latest_script_release = 'https://raw.githubusercontent.com/darealsenor/senor-scripts-related/refs/heads/main/latest_script_release'
local tebex_url = 'https://raw.githubusercontent.com/darealsenor/senor-scripts-related/refs/heads/main/tebex'
...
local function checkNewScriptRelease()
    ...
    if not releaseTimestamp or releaseTimestamp <= 0 then
        return
    end
...
local function processVersionCheck()
    ...
    if needsUpdate(localVerNum, remoteVerNum) then
        lib.print.info(("^3Update available for %s! (current: ^1%s^3, latest: ^2%s^3)"):format(resource,
            installedVersion, fetchedData.version))
        ...
        lib.print.info('^2You are running the latest version of senor-airdrops. Thanks for buying! :)')
    end
...
local function startVersionCheck()
    if not ServerConfig or not ServerConfig.VersionCheck then
        return
    end
    ...
    makeRequest(latest_script_release, 'latest_script_release', requestDelay * 2)
    makeRequest(tebex_url, 'tebex', requestDelay * 3)
end

ServerConfig = ServerConfig or {}
ServerConfig.VersionCheck = ServerConfig.VersionCheck ~= false
