local expectedResourceName = 'senor_kos'
local updateUrl = 'https://raw.githubusercontent.com/darealsenor/senor_kos-fivem/refs/heads/main/version'
...
local function processVersionCheck()
    ...
    if needsUpdate(localVerNum, remoteVerNum) then
        ...
        lib.print.info(('^2You are running the latest version of %s. Thanks for buying! :)'):format(expectedResourceName))
    end
...
CreateThread(function()
    if GetCurrentResourceName() ~= expectedResourceName then
        lib.print.info(('^1[%s] Resource name must be "%s". Rename your resource folder and restart.'):format(expectedResourceName, expectedResourceName))
